...
export const canAccessToEmail = async (
	userId: string,
	organizationId: string,
): Promise<boolean> => {
	...
	return member.canAccessToEmail;
};

export const checkEmailDomainAccess = async (
	userId: string,
	emailDomainId: string | null,
	organizationId: string,
	action: "access" | "create" | "delete",
): Promise<void> => {
	const member = await findMemberById(userId, organizationId);
	...
	switch (action) {
		case "create":
			hasPermission = member.canCreateEmailDomains;
			break;
		case "delete":
			hasPermission = member.canDeleteEmailDomains;
			break;
		case "access":
			hasPermission = member.canAccessToEmail;
			break;
	}
	...
	if (!hasPermission) {
		throw new TRPCError({
			code: "UNAUTHORIZED",
			message: `Permission denied: ${action} email domain`,
		});
	}
	...
	if (emailDomainId) {
		...
		if (domain.organizationId !== organizationId) {
			throw new TRPCError({
				code: "FORBIDDEN",
				message: "Email domain not in your organization",
			});
		}

		// For members, also check if domain is in accessedServices
		if (action === "access" || action === "delete") {
			const hasServiceAccess = member.accessedServices.includes(emailDomainId);
			if (!hasServiceAccess) {
				throw new TRPCError({
					code: "UNAUTHORIZED",
					message: "You don't have access to this email domain",
				});
			}
		}
	}
...
export const checkEmailAccountAccess = async (
	userId: string,
	...
	switch (action) {
		case "create":
			hasPermission = member.canCreateEmailAccounts;
			break;
		case "delete":
			hasPermission = member.canDeleteEmailAccounts;
			break;
		case "access":
			hasPermission = member.canAccessToEmail;
			break;
	...
	if (!hasPermission) {
		throw new TRPCError({
			code: "UNAUTHORIZED",
			message: `Permission denied: ${action} email account`,
		});
	}

	if (emailAccountId) {
		const account = await findEmailAccountById(emailAccountId);
		const domain = account.emailDomain as any;
		if (domain.organizationId !== organizationId) {
			throw new TRPCError({
				code: "FORBIDDEN",
				message: "Email account not in your organization",
			});
		}
	}
};

export const checkEmailForwardAccess = async (
	userId: string,
	emailForwardId: string | null,
	organizationId: string,
	action: "access" | "create" | "delete",
): Promise<void> => {
	const member = await findMemberById(userId, organizationId);
	...
	if (!member.canManageEmailForwards) {
		throw new TRPCError({
			code: "UNAUTHORIZED",
			message: `Permission denied: ${action} email forward`,
		});
	}

	if (emailForwardId) {
		const forward = await findEmailForwardById(emailForwardId);
		const domain = forward.emailDomain as any;
		if (domain.organizationId !== organizationId) {
			throw new TRPCError({
				code: "FORBIDDEN",
				message: "Email forward not in your organization",
			});
		}
	}
};

export const checkEmailAliasAccess = async (
	userId: string,
	...
	if (!member.canManageEmailForwards) {
		throw new TRPCError({
			code: "UNAUTHORIZED",
			message: `Permission denied: ${action} email alias`,
		});
	}

	if (emailAliasId) {
		const alias = await findEmailAliasById(emailAliasId);
		const domain = alias.emailDomain as any;
		if (domain.organizationId !== organizationId) {
			throw new TRPCError({
				code: "FORBIDDEN",
				message: "Email alias not in your organization",
			});
		}
	}
};
